...
public:
    ...
    Order(unsigned init_id, Client init_client, std::unique_ptr<ITariff> init_tariff, float init_mass)
...
public:
    ...
    void add_tariff(const std::unique_ptr<ITariff>& element) {
    ...
    std::unique_ptr<ITariff> get_tariff(int num) {
        if (num >= tariffs.size()) {
            throw std::out_of_range("Index out of bounds");
        }
        return tariffs[num];
    ...
    std::unique_ptr<ITariff> findCheapestTariff() const {
...
private:
    OrderMaster() {
        ...
        std::vector<std::unique_ptr<ITariff>> tariffs;
        size_t size = 0;
    }
    ~OrderMaster() {}
